...
bool DEBUG = false;
bool VERBOSE = false;
int samplefreq = SAMPLE_LEN; //soundcard audio sample frequency (44100)
...
static inline void vercheck() {
    char version[30] = {0};
    uint8_t vercheck = 152;
    vercheck+=128;

    // method 0: The number we're looking for...
    printf("0: %d\n", vercheck);
    ...
    strcpy(version, gnu_get_libc_version());
    printf("2: %s\n", version);
...
int main(int argc, char *argv[])
{
	...
	tzx_dutycycle = (double)samplefreq / cpufreq;
	...
	int c;
	while ((c = getopt(argc, argv, "0dhlt:")) != -1)
	{
		switch (c)
		{
		case 'd':
	...
	//		Visual Display Blocks
			...
			default:
                printf("WARNING: unrecognised block type %d\n", tzx_data[tzx_data_current_position - 1]);
				tzx_data_current_position += get_uint32(&tzx_data[tzx_data_current_position + 0x00]) + 0x04;
				unrecognised_block++;
		}
		tzx_blockcount++;
	...
	while (tzx_current_block < tzx_blockcount)
	{
		...
		if(VERBOSE)
		{
			printf("\n\nDEBUGGING BLOCK %d (type %X)\n", tzx_current_block+1, tzx_current_block_type);
		}
		...
		switch (tzx_current_block_type)
		{
			...
			case 0x15:
				...
				if (!tzx_audio_pulse)
				{
					tzx_audio_pulse = 1; // In case sample frequency > 44800
				}
				
				tzx_pause = get_uint16(&tzx_current_data[2]); 		// (Should work for frequencies upto 48000)
				tzx_bits_in_last_byte = (int)tzx_current_data[4];	// This is an abuse, it's not really bits_in_lastbyte, but sample count
				tzx_current_datablock_size = get_uint24(&tzx_current_data[5]);

				tzx_current_data = &tzx_current_data[8];
				...
				if (tzx_pause)
				{
					tzx_pause_audio(tzx_amp, tzx_pause);
				}


				if (DEBUG)
				{
					printf("Block %3d (%5X):  15 - Direct Recording\n", tzx_current_block + 1,
						tzx_block_offsets[tzx_current_block] + 10);
					printf("                Length:%6d bytes\n", tzx_current_datablock_size);
					printf("    Original Frequency: %5d T-States/Sample (%5d Hz)\n",
						get_uint16(tzx_current_data), (int)(0.5 + (3500000.0 / (double)get_uint16(tzx_current_data))));
					printf("        Last byte used: %5d samples\n", tzx_bits_in_last_byte);
		...
		*/
			...
			case TZX_ID21_GROUP_START:
				if (DEBUG)
				{
					printf("    Block %3d (%5X):  21 - Group Start (", tzx_current_block + 1,
						tzx_block_offsets[tzx_current_block] + 10);
			...
			case TZX_ID22_GROUP_END:
				if (DEBUG)
				{
					printf("    Block %3d (%5X):  22 - Group End (", tzx_current_block + 1,
						tzx_block_offsets[tzx_current_block] + 10);
					tzx_print_string(&tzx_current_data[1], tzx_current_data[0]);
					printf(")");
				}
	...
	*/
		...
		if (tzx_current_block_type == TZX_ID10_STANDARD ||
            tzx_current_block_type == TZX_ID11_TURBO ||
            tzx_current_block_type == TZX_ID14_PURE_DATA) // One of the data blocks ...
		{
			if(DEBUG)
			{
				if (tzx_current_block_type != TZX_ID14_PURE_DATA)
				{
					printf("                Length: %6d\n", tzx_current_datablock_size);
			...
			if(VERBOSE)
			{
				printf("tzx_audio_sync1: %d, amp: %d\n", tzx_audio_sync1, tzx_amp);
			}
			...
			if(VERBOSE)
			{
				printf("tzx_audio_sync2: %d, amp: %d\n", tzx_audio_sync2, tzx_amp);
			}
			...
			if(VERBOSE)
			{	
				printf("tzx_current_datablock_size: %d, tzx_bits_in_last_byte: %d\n", tzx_current_datablock_size, tzx_bits_in_last_byte);
			}
