...
int pause;              // Pause after current block (in 1s/10)
...
int ConvSB(int n) { return((int) (0.5+(cycle* (double) n))); }
...
void GetC64ROMName(char *name, char *data)
{
  char d;
...
void GetC64StandardTurboTapeName(char *name, char *data)
{
  char d;
...
void IdentifyC64ROM(int pos, char *data, int type)
{
  char name[255];
...
void IdentifyC64Turbo(int pos, char *data, int type)
{
  char name[255];
...
void Identify(int len, char *temp, int type)
{
int n,s;
...
char *GetCheckSum(char *data,int len)
{
...
else { sprintf(pstr,"Wrong, should be %3d ($%02X)",c,c); return(pstr); }
}
...
while (s[n])
    ...
    if (s[n]==13)
        {
        d[m]=13; d[m+1]=10;         // Here is the MS-DOS output for line-end
        m+=2;
        for (i=0; i<spaces; i++)
            {
            d[m]=' ';
...
{
    printf("\nZXTape Utilities - Play TZX , TZX to VOC Converter and TZX Info v0.60b\n");
    if (argc<2)
    {
        printf("\nUsage: PLAYTZX [switches] FILE.TZX [OUTPUT.VOC]\n\n");
        printf("       Switches:  /voc      Create a .VOC file instead of SB output\n");
        ...
        printf("                  /reverse  Inverts the output\n");
        exit();
    }
    ...
    for (n=1; n<argc; n++)
    {
        if (argv[n][0]=='/')
        {
            switch (argv[n][1])
            {
                case 'v':   voc=1; freq=30303; break;
                case 'i':   info=1; break;
                case 'o':   info=2; break;
                case 'p':   pages=1; break;
    ...
    if (voc && info) error("Both /voc AND /info (or /one) switches selected !");
    if (files==0) error("No Files specified !");
    if (files==1) { strcpy(fout,finp); ChangeFileExtension(fout,"VOC"); }
    if (nfreq) freq=nfreq;
    //
    if (!voc && !info) { if (!SB_Detect()) Error("SoundBlaster could not be detected!"); }
    ...
    // Start reading file...
    read(fh,mem,10); mem[7]=0;
    if (strcmp(mem,"ZXTape!")) { free(mem); Error("File is not in ZXTape format!"); }
    ...
    if (starting>1)
        ...
        if (starting>numblocks)
            { free(mem); Error("Invalid Starting Block"); }
        curr=starting-1;
        }
    if (ending>0)
        {
        if (ending>numblocks || ending<starting)
            { free(mem); Error("Invalid Ending Block"); }
        numblocks=ending;
        }
    if (!info)
        {
    ...
    if (!info) { if (!voc) InitSB(); else InitVOC(); }
    amp=LOAMP;
    ...
    while (curr<numblocks)
        {
        if (!info) { if (draw) printf("Block %3d:",curr+1); }
        else    {
                if (info==2 && draw)
                    {
                    if (pages)
        ...
        switch (id)
            ...
            case 0x10:  pause=Get2(&data[0]);
                        datalen=Get2(&data[2]);
                        data+=4;
                        if (data[0]==0x00) pilot=8064; else pilot=3220;
            ...
            case 0x11:  sb_pilot=ConvSB(Get2(&data[0]));
                        ...
                        break;
            // Pure Tone
            case 0x12:  sb_pilot=ConvSB(Get2(&data[0]));
                        pilot=Get2(&data[2]);
                        if (info!=1)
                            {
                            ...
                            if (info!=2)
                                {
                                while (pilot) { PlaySB(amp,sb_pilot); ToggleAmp(); pilot--; }
                                }
                            }
                        else
                            {
                        ...
                        break;
            // Sequence of Pulses
            case 0x13:  pilot=(int) data[0]; data++;
                        if (info!=1)
                            {
                            ...
                            if (info!=2)
                                ...
                                while(pilot)
                                    ...
                                    PlaySB(amp,sb_pulse); ToggleAmp(); pilot--; data+=2;
                                    }
                                }
                            }
                        else
                        ...
                        break;
            // Pure Data
            case 0x14:  sb_pilot=pilot=sb_sync1=sb_sync2=0;
                        ...
                        break;
            // Direct Recording
            case 0x15:  sb_pulse=ConvSB(Get2(&data[0]));
                        ...
                        pause=Get2(&data[2]);               // (Should work for frequencies upto 48000)
                        lastbyte=(int) data[4];
                        datalen=Get3(&data[5]);
                        if (info!=1)
                            {
                            ...
                            if (info!=2)
                                ...
                                while (datalen)
                                    {
                                    if (datalen!=1) bitcount=8; else bitcount=lastbyte;
                                    databyte=data[datapos];
                                    while (bitcount)
                                        {
                                        if (databyte&0x80) amp=HIAMP; else amp=LOAMP;
                                        PlaySB(amp,sb_pulse);
                                        databyte<<=1;
                                        bitcount--;
                                        }
            ...
            case 0x20:  pause=Get2(&data[0]);
                        ...
                        if (pause)
                            ...
                            if (info!=1)
                                ...
                                if (info!=2) { PauseSB(amp,pause); amp=LOAMP; }
                                }
                            else
                                {
                            ...
                            if (info!=1)
                                ...
                                if (!voc)
                                    ...
                                    {
                                    if (draw) printf("    Stop the tape command!\n");
                                    if (info!=2) { PauseSB(amp, 5000); amp=LOAMP; }
                                    }
                                }
                        ...
                        break;
            // Group Start
            case 0x21:  CopyString(pstr,&data[1],data[0]);
            ...
            case 0x23:  jump=(signed short) (data[0]+data[1]*256);
                        ...
                        break;
            // Loop Start
            case 0x24:  loop_start=curr; loop_count=Get2(&data[0]);
            ...
            case 0x25:  if (info!=1)
                        ...
                        break;
            // Call Sequence
            case 0x26:  call_pos=curr; call_num=Get2(&data[0]); call_cur=0;
                        if (info==1)
                            {
            ...
            case 0x2A:  if (info==1)
                        ...
                        break;
            // Description
            case 0x30:  CopyString(pstr,&data[1],data[0]);
                        if (info!=1) { if (draw) printf("    Description: %s\n",pstr); }
                        else    {       sprintf(tstr,"Block %3d (%5X):  30 - Description: %s\n\n",curr+1,block[curr]+10, pstr); line++; writeout(tstr); }
                        break;
            // Message
            ...
            default:    if (info!=1) { if (draw) printf("    Unknown block %02X !\n", id); }
                        else    {   sprintf(tstr,"Block %3d (%5X):  %02X Unknown Block \n\n",curr+1,block[curr]+10,id); line++; writeout(tstr); }
                        break;
            }
        if (info!=1 && (id==0x10 || id==0x11 || id==0x14))    // One of the data blocks ...
            ...
            if (info!=2)
                {
                while (pilot) { PlaySB(amp,sb_pilot); ToggleAmp(); pilot--; }   // Play PILOT TONE
                if (sb_sync1) { PlaySB(amp,sb_sync1); ToggleAmp(); }            // Play SYNC PULSES
                if (sb_sync2) { PlaySB(amp,sb_sync2); ToggleAmp(); }
                datapos=0;
                while (datalen)                                                 // Play actual DATA
                    ...
                    while (bitcount)
                        {
                        if (databyte&0x80) sb_bit=sb_bit1; else sb_bit=sb_bit0;
                        PlaySB(amp,sb_bit); ToggleAmp();
                        PlaySB(amp,sb_bit); ToggleAmp();
                        databyte<<=1;
                        bitcount--;
                        }
                ...
                if (pause) { PauseSB(amp,1); amp=LOAMP; if (pause>1) PauseSB(amp,pause-1); }
                }
            }
        if (info!=1 && id==0x16)    // C64 ROM data block ...
            ...
            if (info!=2)
                ...
                sb_sync1=ConvSB(sb_sync1); sb_sync2=ConvSB(sb_sync2);
                sb_bit1_f=ConvSB(sb_bit1_f); sb_bit1_s=ConvSB(sb_bit1_s);
                sb_bit0_f=ConvSB(sb_bit0_f); sb_bit0_s=ConvSB(sb_bit0_s);
    ...
    //            if (pause) { PauseSB(amp,1); amp=LOAMP; if (pause>1) PauseSB(amp,pause-1); }
                ...
                if (pause)
                {
                    PauseSB(amp, pause/2); ToggleAmp();
                    PauseSB(amp, (pause/2)+(pause%2)); ToggleAmp();
                }
                }
            }
